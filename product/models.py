...
class TableReservation(models.Model):
    ...
    condition = models.CharField(
        ...
        null=False, blank=False, default=NEW, verbose_name="Состояние"
    )
    status = models.CharField(
        max_length=20, choices=STATUS,
        null=False, blank=False, default=NOT_CONFIRMED, verbose_name="Статус"
    )
    telegram_user_id = models.PositiveSmallIntegerField(
        null=False, blank=False, verbose_name="Telegram Id пользователя"
    )
    ...
    def __str__(self):
        return f"{self.telegram_user_id} - " \
               f"{self.table_number}. {self.condition}. {self.status}"
