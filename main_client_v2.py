...
import time
from product.models import TelegramUser, Basket, Aboutus, Category
    ...
    if m.contact is not None:
        ...
        bot.send_message(
            m.chat.id, f'Пользователь *{m.contact.first_name}'
                       ...
                       f'{m.contact.phone_number}* добавлен в базу'
                       f' \U0001F4BB', parse_mode="Markdown")
        start(m)
        ...
        if not Basket.objects.filter(
            ...
            bot.send_message(
                m.chat.id, '<i>Корзина пуста, перейдите в</i> '
        ...
        for menu in response_menu:
            ...
            if call.data == menu['category']:
                ...
                bot.send_message(
                    ...
                    f"*{menu['category']}, {menu['product_name']} "
                ...
                if not Basket.objects.filter(
                    ...
                    bot.send_photo(
                        call.message.chat.id, photo, caption=text_menu(menu),
                        reply_markup=keyboard,
                        parse_mode="Markdown")
                ...
                if not Basket.objects.filter(
                    ...
                    bot.answer_callback_query(
                        ...
                        text=f'"{basket.product.product_name}" '
                             f'добавлено в корзину \n '
                             f'Общая количество {basket.amount}')
