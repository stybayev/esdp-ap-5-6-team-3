...
def subtract_meals(basket, menu):
    """
        Функция для изменения количества продукта в заказе, а
        именно убавление количества продукта в заказе
    """
    basket.amount -= 1
...
def button_menu(keyboard, basket):
    """
        Функция для инлайн кнопок, добавление и убавление
        количества продукта в меню (кнопка Меню)
    """
    add_menu = types.InlineKeyboardButton(
        text=f"\U00002795\U0001F371Добавить в корзину",
...
def bot_message(m):
    ...
    if m.contact is not None:
        ...
        поступление бронь стола с url инлайн кнопкой. После вызывается
        главный меню маркап кнопок (Меню, Корзина, О Нас, Оформить
        заказ, Статус заказа, Выполненные заказы, Забронировать столик,
        Оценить ресторан)
        """
        ...
        и выводится главный меню маркап кнопок (Меню, Корзина,
        О Нас, Оформить заказ, Статус заказа, Выполненные заказы,
...
def callback_inline2(call):
    ...
    if call.data in TIME:
        """
            Если выбранное время в бронирования столика находиться в
            константе TIME, то данное время записывается на временную
    ...
    if call.data == 'order_processing':
        """
            ...
            следующими параметрами (№заказа, телеграммID, дата создание),
            после все продукты с временного базы модель «Basket»
            переносится на постоянную базу модель «BasketToOrder»
    ...
    if call.data[:16] == f'completed_orders':
        """
            ...
            выполненных заказов в виде инлайн кнопок с указанием №заказа,
            общая сумма и дата выполненных операции. После нажатие на эти
            инлайн кнопки клиенту отправляется PDF файл с указанием номер
            заказа, список продуктов с количеством и суммой, итоговая сумма
            продукта и итоговая сумма с процентом мерчанта.  Сперва PDF файл
            формируется в проекте и сохраняется в корни проекта в папке PDF,
            после отправляется клиенту на телеграмм бот, после с папки PDF
            удаляется файл
        """
