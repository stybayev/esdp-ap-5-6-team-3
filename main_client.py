...
def subtract_meals(basket, menu):
    """
        Функция для изменения количества продукта в заказе,
        а именно убавление количества продукта в заказе
    """
    basket.amount -= 1
...
def add_meals(basket, menu):
    """
        Функция для изменения количества продукта в заказе,
        а именно добавление количества продукта в заказе
    """
    basket.amount += 1
    basket.product_total_price += menu['price']
    basket.save()
...
def button_basket(keyboard, basket):
    """
        Функция для инлайн кнопок, добавление и убавление
        количества продукта в корзине (кнопка Корзина)
    """
...
def button_menu(keyboard, basket):
    """
        Функция для инлайн кнопок, добавление и
        убавление количества продукта в меню (кнопка Меню)
    """
    add_menu = types.InlineKeyboardButton(
        text=f"\U00002795\U0001F371Добавить в корзину",
...
def start(m):
    """
        ...
        проверяет клиента на регистрацию
    """
...
def bot_message(m):
    ...
    if m.contact is not None:
        """
            Если клиент не зарегистрирован в Базе, то после нажатие
            на кнопку 'Поделиться номером!' данные клиента (Имя, Фамилия,
            Телефон, Телеграмм ID) сохраняется в БД модели TelegramUser
        """
        ...
        """
            ...
            список инлайн кнопки категории продуктов
        """
        ...
        """
        После нажатие на маркап кнопку Бронировать в базе модель
        TableReservation создается бронь столика со статусном «Новый»
        и телеграмм боту мерчанта отправляется уведомление о
        поступлениебронь стола с url инлайн кнопкой.После вызывается
        главный меню маркап кнопок (Меню, Корзина, О Нас, Оформить
        заказ, Статус заказа, Выполненные заказы, Забронировать
        столик, Оценить ресторан)
        """
        ...
        """
            После нажатие на маркап кнопку 'Оформить заказ', если
            корзина не пуста то выводится информация о продукте,
        ...
        """
            После нажатие на маркап кнопку 'Статус заказа' выводится
            активные заказы со статусом еще не принято или в обработке,
            а также внизу заказа выводится инлайн кнопка 'Детальный
            просмотр заказа №'
        """
        for orders in ShoppingCartOrder.objects.filter(
        ...
        """
            После нажатие на маркап кнопку 'Оценить ресторан'
            опросник по 5 бальной шкале
        """
        bot.send_poll(
        ...
        """
            После нажатие на маркап кнопку Отменить оценку отменяет
            оценку очищая временную оценку и вызывает главный меню
            маркап кнопок (Меню, Корзина, О Нас, Оформить заказ,
            Статус заказа, Выполненные заказы, Забронировать столик,
            Оценить ресторан)
        """
        ...
        for orders in ShoppingCartOrder.objects.filter(
                telegram_user_id_id=m.from_user.id):
            detail_view_order = types.InlineKeyboardButton(
                text=f"Заказ №{orders.pk} на сумму "
                     f"{orders.sum_product_total_price()} "
        ...
        """
        После нажатие на маркап кнопку Оставить оценку оценки и
        отзывы клиента сохраняется в БД модель CustomerFeedback
        и выводится главный меню маркап кнопок (Меню, Корзина, О Нас,
        Оформить заказ, Статус заказа, Выполненные заказы,
        Забронировать столик, Оценить ресторан)
        """
        ...
        """
            Сохраняет отзывы клиента на временную
            словарь customer_feedback = {}
        """
...
def callback_inline2(call):
    ...
    if call.data in TIME:
        """
            Если выбранное время в бронирования столика находиться
            в константе TIME, то данное время записывается на временную
            словарь database для дальнейшего обработки данных
        """
    ...
    if call.data == 'order_processing':
        """
            После нажатие на инлайн кнопку «Оформить заказ» в базе
            модель «StatusShoppingCartOrder» создается заказ с
            следующими параметрами (№заказа, телеграммID, дата
            создание), после все продукты с временного базы модель
            «Basket» переносится на постоянную базу модель «BasketToOrder»
            с добавлением параметра заказа (т.е. ID заказа). Следом
            телеграмм боту мерчанта отправляется уведомление с инлайн
            кнопкой, в инлайн кнопке указывается ссылка на созданную заказ.
        """
    ...
    if call.data[:16] == f'completed_orders':
        """
            В маркап кнопке «Выполненные заказы» отображается истории
            выполненных заказов в виде инлайн кнопок с указанием
            №заказа, общая сумма и дата выполненных операции. После
            нажатие на эти инлайн кнопки клиенту отправляется PDF файл
            с указанием номер заказа, список продуктов с количеством
            и суммой, итоговая сумма продукта и итоговая сумма с
            процентом мерчанта.  Сперва PDF файл формируется в проекте
            и сохраняется в корни проекта в папке PDF, после отправляется
            клиенту на телеграмм бот, после с папки PDF удаляется файл
        """
        if BasketToOrder.objects.filter(telegram_user_id_id=call.from_user.id,
            ...
            for baskets in BasketToOrder.objects.filter(
                ...
                pdf.cell(120, 10,
                         txt=f'{baskets.product.product_name}: '
                             f'{baskets.product.price} тг. x '
                             f'{baskets.amount} шт. = '
                             f'{baskets.product_total_price} тенге',
                         ln=1, align="C")
    ...
    if call.data != '\U0001F4D6\U0001F372\U0001F354Меню':
        for menu in response_menu:
            if call.data == menu['category']:
                """
                    После нажатие на инлайн кнопку «Категории продукта»
                    (например: Напитки) отображается список продуктов
                    под категории с указанием Фото продукта, Наименование,
                    Цена и Описание продукта, а также инлайн кнопка
                    «Добавить в корзину».
                    Если продукт добавлен в корзину, то еще отображается
                    количество и сумма продукта, а также две инлайн кнопки
                    «Добавить в корзину» и «удалить с корзины»
                """
                for response_category in response_categories:
                ...
                """
                    Добавляет либо увеличивает количества
                    продукта и сумму в Меню
                """
                if not Basket.objects.filter(
                        product_id=menu['id'],
                        telegram_user_id_id=call.from_user.id):
                ...
                """
                    После нажатие на инлайн кнопку «Изменить заказ - Корзина»
                    открывается корзина для дальнейшего редактирование списка
