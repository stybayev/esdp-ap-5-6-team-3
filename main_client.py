...
import time
from product.models import TelegramUser, Basket, Aboutus, \
    BasketToOrder, ShoppingCartOrder, StatusShoppingCartOrder, \
    MerchantTelegramUser, TableReservation
...
def text_basket(basket):
    basket_text = (f"_Наименование_: *{basket.product.id}-"
...
def button_basket(keyboard, basket):
    add_menu = types.InlineKeyboardButton(
        text=f"\U00002795\U0001F371 Добавить в корзину",
    ...
    if TelegramUser.objects.filter(user_id=m.from_user.id):
        ...
        keyboard.add(*[types.KeyboardButton(bot_message) for bot_message in
                       ['\U0001F4D6\U0001F372\U0001F354Меню',
                        '\U0001F371Корзина']])
        keyboard.add(
            *[types.KeyboardButton(bot_message) for
              bot_message in ['\U0001F4DCО Нас', '\U0001F45DОформить заказ']])
        keyboard.add(*[types.KeyboardButton(bot_message) for bot_message in
...
def bot_message(m):
    ...
    if m.contact is not None:
        ...
        bot.send_message(
            m.chat.id, f'Пользователь '
            f'*{m.contact.first_name} {m.contact.last_name} '
            f'{m.contact.phone_number}* '
            f'добавлен в базу \U0001F4BB', parse_mode="Markdown")
        start(m)
        ...
        keyboard.add(*[types.KeyboardButton(bot_message) for bot_message in
                       ['\U0001F4D6\U0001F372\U0001F354Меню',
                        '\U0001F371Корзина']])
        keyboard.add(
            *[types.KeyboardButton(bot_message) for bot_message
              in ['\U0001F4DCО Нас', '\U0001F45DОформить заказ']])
        keyboard.add(*[types.KeyboardButton(bot_message) for bot_message in
                       ['\U0001F55CСтатус заказа',
                        '\U0001F51AВыполненные заказы']])
        keyboard.add(*[types.KeyboardButton(bot_message) for bot_message in
        ...
        for basket in Basket.objects.all():
            if m.from_user.id == basket.telegram_user_id_id:
                ...
                bot.send_photo(m.chat.id, photo, caption=text_basket(basket),
                               reply_markup=keyboard,
                               parse_mode="Markdown")
        ...
        if not Basket.objects.filter(telegram_user_id_id=m.from_user.id):
            ...
            bot.send_message(m.chat.id,
                             ...
                             '<ins><b>Меню</b></ins> <i>для заказа блюд</i>',
            ...
            for basket in Basket.objects.all():
                if basket.telegram_user_id_id == m.from_user.id:
                    ...
                    bot.send_message(m.chat.id,
                                     ...
                                     parse_mode='Markdown')
            bot.send_message(
                m.chat.id, f"_Итого общая сумма продукта:_ *"
                f"{total_sum}* \n"
                f"_10% за обслуживание:_ *"
                f"{(total_sum * 10) / 100}* \n\n"
                f"Итого общая сумма: *"
                f"{((total_sum * 10) / 100) + total_sum}*",
                reply_markup=keyboard, parse_mode='Markdown')
...
def callback_inline(call: CallbackQuery):
    ...
    database.setdefault(call.from_user.id,
...
def callback_inline(call):
    if call.data in order(call):
        ...
        if BasketToOrder.objects.filter(
                ...
                order=order_pk):
            bot.send_message(
                call.message.chat.id, f"Заказ *№{order_pk}: "
                                      f"еще не принято в обработку*",
                parse_mode='Markdown')
        elif BasketToOrder.objects.filter(
                telegram_user_id_id=call.from_user.id, order__status=2,
                order=order_pk):
            bot.send_message(call.message.chat.id, f"Заказ *№{order_pk}: "
                                                   f"принято в обработку*",
                             parse_mode='Markdown')
        for basket in BasketToOrder.objects.filter(
            ...
            bot.send_message(
                ...
                f"_{basket.product.price}_тг. = _"
                f"{basket.product_total_price}_ тенге",
                parse_mode='Markdown')
        bot.send_message(
            call.message.chat.id,
...
количество людей: {database[call.from_user.id]['persons']}
    ...
    if call.data == 'order_processing':
        ...
        bot.send_message(
            ...
            f"_Итого общая сумма продукта:_ "
            f"*{total_sum}* \n"
            f"_10% за обслуживание:_ "
            f"*{(total_sum * 10) / 100}* \n\n"
            f"Итого общая сумма: "
        ...
        for users in MerchantTelegramUser.objects.all():
            ...
            merchant_bot.send_message(
                users.user_id, f'поступил заказ с номером '
                f'*№{shopping_cart_orders.id}* \n'
                f'сумма заказа '
                f'*{((total_sum * 10) / 100) + total_sum}* тенге',
                reply_markup=keyboard, parse_mode='Markdown')
    ...
    if call.data != '\U0001F4D6\U0001F372\U0001F354Меню':
        for menu in response_menu:
            ...
            if call.data == menu['category']:
                for response_category in response_categories:
                    # if Product.objects.filter(available='Есть',
